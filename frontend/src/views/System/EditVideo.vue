...
<script setup> 
  ...
  import { getVideoOneApi, modifyVideoApi, videotitleImgUploadApi, videoUploadApi } from "@/api/modules/video";
  ...
  const onSubmit = (formEl) => {
    ...
    formEl.validate((valid) => {
      if (valid) {
        modifyVideoApi({videoId: searchInfo.videoId, title: searchInfo.title, videoTitleImg: searchInfo.videoTitleImg, 
  ...
  const getarticleInfo = async() => {
    const data = await getVideoOneApi({videoId: searchInfo.videoId});
    if(data.code == '0000000'){
      searchInfo.title = data.data.title;
      searchInfo.author = data.data.author;
      searchInfo.videoTitleImg = data.data.videoTitleImg;
      searchInfo.videoUrl = data.data.videoUrl;
    }
  }
