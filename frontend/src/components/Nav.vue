<template>
    <el-aside class="nav" width="auto">
      ...
      <div class="logo tuwenfenkai" @click="clickLogo" :class="{collapse:isCollapse}">
      ...
      <el-scrollbar>
        <el-menu
          ...
          :collapse="isCollapse"
        >
          <div v-for="item in pageTree" :key="item.path">
            <el-sub-menu
              :index="item.path"
            ...
            >
              ...
              >
                ...
                >
                  <span class="iconfont" :class="sonItem.meta.icon"></span>
                  {{ sonItem.meta.title }}
                </el-menu-item>
              </el-menu-item-group>
            </el-sub-menu>
            <el-menu-item :index="item.path" v-else @click="itemClickHandler">
              <span class="iconfont" :class="item.meta.icon"></span>
              {{ item.meta.title }}
            </el-menu-item>
          </div>
...
export default defineComponent({
  setup() {
    ...
    const state = reactive({
      ...
      // 当前选中菜单
      activeRouterPath: store.state.system.subMenuRouter.path || store.state.system.mainMenuRouter.path,
      // 菜单展开收缩
     ...
     * @author: 吴志远
     * @todo: 监听路由树是否改变，刷新路由
     * @Date: 2021-11-22 10:01:22
     */    
    watch(() => store.state.system.routerTree,(val) => {
        state.pageTree = val;
      }
    );
     ...
     * @author: 吴志远
     * @todo: 监听一级菜单对象改变
     * @Date: 2021-11-22 10:02:25
     */    
    watch(()=>store.state.system.mainMenuRouter, (val) => {
      // 如果二级菜单为空，当前选中为一级菜单
     ...
     * @author: 吴志远
     * @todo: 监听二级菜单对象改变
     * @Date: 2021-11-22 10:05:25
     */    
    watch(()=>store.state.system.subMenuRouter, (val) => {
      if(store.state.system.mainMenuRouter.path !== '' && val.path !== '') {
        state.activeRouterPath = val.path || store.state.system.mainMenuRouter.path;
      } 
    },{
      deep: true
    })
